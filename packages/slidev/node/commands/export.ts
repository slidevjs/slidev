...
import { PDFDocument, PageSizes, rgb, PDFString, PDFDict, asPDFName, PDFName } from 'pdf-lib'
...
export async function exportSlides({
  ...
  async function go(no: number | string, clicks?: string, query: string = 'print') {
  ...
  async function mergeSlidesWithNotes(
    ...
    for (let i = 0; i < numSlides; i++) {
      ...
      try {
        ...
        console.log("Could not embed note as page does not exist: " + error);
      }

      /* add links for slides */
      const annots = pdfSlidePages[i].node.Annots();

      let newLinkAnnotations = []; // Initialize an empty array to accumulate new link annotations
      ...
      try {
        annots?.asArray().forEach((a) => {
          ...
          if (subtype === "/Link") {
            ...
            //console.log(`Link ${uri}, Position: ${rect}`);
      ...
      try {
        ...
        notesAnnots?.asArray().forEach((a) => {
          ...
          if (subtype === "/Link") {
            ...
            const offsetX =
              currentPage.getWidth() / 2 - noteEmbeddedDims.width / 2;
  ...
  async function genHandoutAndMerge(pdfSlidesPath: string) {
    ...
    slidesData = await fs.readFile(pdfSlidesPath)
    pdfSlides = await PDFDocument.load(slidesData)

    /* 2. Generate notes pdf */
    let notesPath = await genNotesPdfOnePiece()
    ...
    pdf = await mergeSlidesWithNotes(pdfSlides, pdfNotes, pdfCover)
