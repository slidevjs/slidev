...
export default function setupShortcuts() {
  ...
  const injection_arg: { nav: NavOperations; defaultShortcuts: ShortcutOptions[] } = {
    nav: {
      next,
      prev,
      nextSlide,
  ...
  return { customShortcuts: injection_return, defaultShortcuts: injection_arg.defaultShortcuts }
}
